...
// Render 1 product on the product page
function renderProduct() {
    const parameters = new URLSearchParams(window.location.search);
    const productId = parseInt(parameters.get('id'));
    ...
    if (product) {
        ...
        document.getElementById("products").innerHTML = "<p>Product not found.</p>";
    }
}
...
function addProductCart(productId) {
    const product = products.find(p => parameter.id === productId);
    if (product) {
        const cart = getCartLS();
        cart.push(product);
